 ...
 *           2.1.2) MyPoint constructor with parameters x, y - public
 *           MyPoint(int x, int y)
 ...
 *           2.1.5) getDistance implementation to calculated distance between
 *           another point
 ...
 *           3.1.1) MyCircle constructor with parameters x, y, radius - public
 *           MyCircle(int x, int y, int radius)
 *           3.1.2) MyCircle constructor with parameters MyPoint, radius -
 ...
 *           3.1.5) Implementation of a function that given a point verifies if
 *           it is located inside the circle
 *           public boolean isPointInsideCircle(MyPoint point)
 *           3.1.6) Implementation of a intersects function that given another
 *           circle, informs if the circles overlap
 *           public boolean intersects(MyCircle otherCircle)
 *           4) Ex2.main additions
 *           4.1) Create 3 circles (mc1, mc2, mc3) and making mc1 and mc2
 *           overlap, at the same time mc3 does not overlap
 *           4.2) Create point mp1 located inside mc1 and verify and print if it
 *           is located inside the circle
 *           4.3) Create point mp2 located outside mc2 and verify and print if
 *           it is located inside the circle
 ...
 *           5.1) Make changes to the existing code in order to improve the
 *           'visual' experience
 *
 *           <p>
...
public class Ex2 {
         ...
         * 
         * @implNote printPointInsideCircleVerification() method takes the needed
         *           parameters in
         *           order to verify if a point is located inside a circle and print it
         *           in a
         *           standard way to the screen
         *
         * @param (MyCircle circle) (Circle)
         * @param (MyPoint  point) (Point to be verified if it is inside the previous
         *                  circle)
         ...
         * 
         * @implNote printCirclesOverlappingVerification() method takes two circles as
         *           parameters in
         *           order to verify they overlap and print it in a standard way to the
         *           screen
         *
         * @param (MyCircle firstCircle) (First circle)
         * @param (MyCircle secondCircle) (Second circle to be verified if it overlaps
         ...
         * @param ()      (No parameters)
         */
        private void AddNewCircleToCanvas(Circle circleInCanvas, String string, int x, int y, int radius) {
                circleInCanvas = new Circle(string, x, y, radius);
                circleInCanvas.setColor(Color.BLACK);
                circleInCanvas.setIsFilled(true);
                circleInCanvas.setFillColor(Color.PINK);
        ...
        public static void main(String[] args) {
                ...
                Circle noIntersectingCircle = new Circle("circle3", mc3.getCenter().getX(), mc3.getCenter().getY(),
                                mc3.getRadius());
                ...
                Line lineBetweenCentersOfCircle1AndCircle3 = new Line("line1_3",
                                mc1.getCenter().getX(), mc1.getCenter().getY(),
                                mc3.getCenter().getX(), mc3.getCenter().getY());
                ...
                Line lineBetweenCentersOfCircle2AndCircle3 = new Line("line2_3",
                                mc2.getCenter().getX(), mc2.getCenter().getY(),
                                mc3.getCenter().getX(), mc3.getCenter().getY());
                ...
                final int CORNER_CIRCLE_RADIUS = 5;

                // Drawing the 4 circles closing the perimeter defined by 400 x 400 pixels
                Circle circleUpperLeftCircle = null;
                ex2.AddNewCircleToCanvas(circleUpperLeftCircle, "UpperLeftCircle",
                                UPPER_LEFT_CORNER_POINT_X, UPPER_LEFT_CORNER_POINT_Y, CORNER_CIRCLE_RADIUS);
                Circle circleUpperRightCircle = null;
                ex2.AddNewCircleToCanvas(circleUpperRightCircle, "UpperRightCircle",
                                UPPER_RIGHT_CORNER_POINT_X, UPPER_RIGHT_CORNER_POINT_Y, CORNER_CIRCLE_RADIUS);
                Circle circleLowerLeftCircle = null;
                ex2.AddNewCircleToCanvas(circleLowerLeftCircle, "LowerLeftCircle",
                                LOWER_LEFT_CORNER_POINT_X, LOWER_LEFT_CORNER_POINT_Y, CORNER_CIRCLE_RADIUS);
                Circle circleLowerRightCircle = null;
                ex2.AddNewCircleToCanvas(circleLowerRightCircle, "LowerRightCircle",
                                LOWER_RIGHT_CORNER_POINT_X, LOWER_RIGHT_CORNER_POINT_Y, CORNER_CIRCLE_RADIUS);
